...
Argos 是一个基于 FastAPI 的每日科技简报与阅读助手。它从多种来源（RSS、Hacker News、Reddit、GitHub 或纯 LLM）聚合内容，使用任意 OpenAI 兼容 LLM 策划结构化摘要，并通过推荐解释、文章级 RAG 追问和反馈驱动的个性化偏好，帮助你从“看到资讯”走到“理解重点”。每个看板独立运行，拥有自己的来源、提示词、角色和推送渠道。

## ✨ 亮点特性

### 📰 每日简报主路径

围绕“看今日简报 → 理解一篇文章 → 追问/收藏/反馈 → 系统学习偏好”的主路径设计。每张资讯卡都会解释为什么推荐，并给出可直接追问的问题。

### 🔍 阅读助手

文章级 RAG 对话支持引用依据、快速导读和建议追问。混合检索管道结合 Bi-Encoder + BM25、**Cross-Encoder 重排序**和 **HyDE 查询重写**。

### 🎯 个性化推荐

显式喜欢/不喜欢反馈 + 关注/屏蔽话题 + 来源偏好 + **用户记忆系统**（持久化偏好和上下文）。越用越懂你，也能看到推荐原因。
...
深度研究、跨文章 RAG、MCP Server、来源健康监控和成本统计仍然保留在工具入口，用于更深入的分析和自托管运维。
