...
class TestProbeCommonFeedPaths:
    ...
    async def test_returns_reachable_paths(self):
        ...
        with patch("app.api.router._test_single_feed", side_effect=fake_test):
...
class TestRsshubCandidateUrls:
    def test_builds_from_platform_entries(self):
        plan = {"candidates": {"rsshub": [
            {"platform": "bilibili_user_video", "uid": "2267573"},
            {"platform": "jike_user", "id": "ABC"},
        ]}}
        with patch("app.api.router.settings", MagicMock(RSSHUB_ENABLED=True)), \
    ...
    def test_disabled_returns_empty(self):
        ...
        with patch("app.api.router.settings", MagicMock(RSSHUB_ENABLED=False)):
            assert router._rsshub_candidate_urls(plan) == []
...
class TestDiscoverRssCandidatesFallback:
    ...
    async def test_falls_back_to_common_paths_when_autodiscovery_empty(self):
        ...
        with patch("app.api.router.tavily_search", AsyncMock(return_value=[]), create=True), \
             patch("app.services.research_service.tavily_search", AsyncMock(return_value=[])), \
             patch("app.api.router._discover_feeds", AsyncMock(return_value=[])), \
             patch("app.api.router._probe_common_feed_paths", AsyncMock(return_value=["https://zhihu.com/feed"])), \
             patch("app.api.router._rsshub_candidate_urls", return_value=[]):
    ...
    async def test_includes_rsshub_urls(self):
        ...
        with patch("app.services.research_service.tavily_search", AsyncMock(return_value=[])), \
             patch("app.api.router._rsshub_candidate_urls", return_value=["https://rsshub.app/jike/user/X"]):
            feeds = await router._discover_rss_candidates(plan)
    ...
    async def test_skips_common_probe_when_autodiscovery_succeeds(self):
        ...
        with patch("app.services.research_service.tavily_search", AsyncMock(return_value=[])), \
             patch("app.api.router._discover_feeds", AsyncMock(return_value=["https://blog.com/atom"])), \
             patch("app.api.router._probe_common_feed_paths", probe), \
             patch("app.api.router._rsshub_candidate_urls", return_value=[]):
            feeds = await router._discover_rss_candidates(plan)
