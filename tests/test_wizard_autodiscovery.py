...
from app.api.router import _parse_feed_links
...
def test_finds_rss_and_atom_links():
    ...
    """
    feeds = _parse_feed_links(html, "https://example.com/blog")
...
def test_resolves_relative_href_against_base():
    ...
    feeds = _parse_feed_links(html, "https://example.com/sub/page")
...
def test_ignores_non_feed_links():
    ...
    """
    assert _parse_feed_links(html, "https://example.com") == []


def test_no_feeds_returns_empty():
    assert _parse_feed_links("<html><head></head></html>", "https://example.com") == []
...
def test_dedupes_repeated_feeds():
    ...
    <link rel="alternate" type="application/rss+xml" href="/feed">
    """
    assert _parse_feed_links(html, "https://example.com") == ["https://example.com/feed"]


def test_respects_limit():
    links = "".join(
        f'<link rel="alternate" type="application/rss+xml" href="/feed{i}">'
        for i in range(10)
    )
    feeds = _parse_feed_links(links, "https://example.com", limit=3)
    assert len(feeds) == 3


def test_malformed_html_never_raises():
    # Garbage input must degrade to [] rather than propagating an exception.
    assert _parse_feed_links("<<<not html>>> <link rel=", "https://example.com") == []
